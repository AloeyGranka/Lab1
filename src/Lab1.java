...
public class Lab1 {
	...
	public String derivative(String expre, String var) {
		int i;
		int count = 0;
		...
		if (expre.contains("*")) {
			if (!expre.contains(var + "^")) {
				...
				while (i != count) {
					begin[i] = expre.indexOf(var, begin[i-1] + 1);
					System.out.println("begin: " + String.valueOf(i) + " " +  String.valueOf(begin[i]));
					i++;
				}
				...
				for (i = 0; i < begin.length; i++) {
					if (begin[i] > 0 && fl) {
						if (begin[i] > (1 + var.length()) * i) {
							sb.delete(begin[i]- (1+var.length())*i - 1, 
									begin[i] - (1+var.length())*i + var.length());
						} else {
							sb.delete(0, var.length() + 1);
						}
						fl = true;
					} else {
